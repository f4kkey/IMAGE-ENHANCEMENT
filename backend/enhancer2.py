...
def bilateral_filter_paper_color_lab(image_lab, sigma_d=3.0, sigma_r=10.0, window_radius=None):
    f = image_lab.astype(np.float64)
    H, W, C = f.shape
    ...
    for i in range(H):
        ...
        for j in range(W):
            ...
            s_fx_fxi = np.exp(- dist2 / (2 * sigma_r**2))

            weights = c_xi_x * s_fx_fxi
            k_x = np.sum(weights)

            if k_x > 1e-12:
                w_norm = (weights / k_x)[..., np.newaxis]  
                h[i, j, :] = np.sum(w_norm * patch, axis=(0, 1))
            else:
                h[i, j, :] = center_val
